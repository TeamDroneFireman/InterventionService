module.exports = function(Intervention) {
  ...
  Intervention.disableRemoteMethod("updateAttributes", false);       // Removes (PUT) /products/:id
  Intervention.disableRemoteMethod('createChangeStream', true);    // removes (GET|POST) /products/change-stream
  ...
  Intervention.beforeRemote('find', function(ctx, unused, next) {
    ...
    // Next is needed to call find method. If you want another response, don't use it
    next();
  });
